...
def test_no_measurement_literal_repeats_a_value_the_profile_already_defines() -> None:
    ...
    ambiguous_arithmetic_literals = {0.0, 0.5, 1.0}
    for metric_id, threshold in profile["thresholds"].items():
        value = threshold["value"]
        if isinstance(value, (int, float)):
            numeric = float(value)
            if numeric not in ambiguous_arithmetic_literals:
                defined.setdefault(numeric, metric_id)
        else:
            for bound in value:
                numeric = float(bound)
                if numeric not in ambiguous_arithmetic_literals:
    ...
    # The expanded profile also contains 0.0 and 0.5 threshold bounds; those values
    # cannot be attributed to a rule by literal equality alone and are excluded above.
    defined.setdefault(
