...
## Geliştirici Kurulumu ve Kanonik Komutlar

Repository kökünde mevcut Python 3.12 ortamını ağ erişimi olmadan editable
kurmak için:

```powershell
.\.venv312\Scripts\python.exe -m pip install -e . --no-deps --no-build-isolation
```

Kanonik uygulama giriş noktaları `tk3d-multiview` ve `tk3d-poomsae`'dir.
`scripts/run_vitpose_multiview_3d.py` ve `scripts/run_poomsae_scoring.py`
doğrudan çağrıları geriye uyumluluk için aynı ince CLI adaptörlerini çalıştırır.
Legacy bileşenlerin açık sınıflandırması
[aktif ve legacy bileşen kaydında](docs/LEGACY_COMPONENTS.md) tutulur.

Model yüklemeden clean-checkout ve aktif araştırma varlıklarını ayıran kontrol:

```powershell
.\.venv312\Scripts\tk3d-check.exe
```

`PARTIALLY_READY`, Tier-1 hazırken model/ZED/calibration gibi dış araştırma
varlıklarının eksik olduğunu belirtir. Tek clean-install yolu, dependency
otoritesi ve iki katmanlı doğrulama politikası
[reproducibility sözleşmesinde](docs/REPRODUCIBILITY.md) açıklanır.

### İsteğe bağlı performans profilleme

Performans ölçümü varsayılan olarak kapalıdır ve bilimsel çıktı sözleşmesini
değiştirmez. Aktif multiview hattında hiyerarşik CPU duvar-zamanı, desteklenen
PyTorch CUDA aşamalarında CUDA Event süresi ve Torch peak VRAM ölçmek için:

```powershell
cd C:\Users\WWWW\Desktop\tk3d
.\.venv312\Scripts\python.exe scripts\run_vitpose_multiview_3d.py --session outputs\poomsae_1_zed2i_20260731_trimmed\source\session.yaml --max-frames 260 --profile-performance --benchmark-window-start 140 --benchmark-window-end 259
```

Poomsae analiz ve sunum/export maliyetlerini, mevcut doğrulanmış 3B pozu tekrar
işlemeden ayrı ölçmek için:

```powershell
cd C:\Users\WWWW\Desktop\tk3d
.\.venv312\Scripts\python.exe scripts\run_poomsae_scoring.py --profile poomsae1_trimmed --profile-performance
```

Her komut yeni bir run oluşturur ve ölçümü run altındaki
`json\performance_report.json` dosyasına yazar. Bu rapor bilimsel artifact
değildir; parent/child süreleri hiyerarşiktir ve üst üste toplanmamalıdır. ZED
SDK belleği Torch VRAM değerine dahil değildir; sistem RAM/CPU ölçümü mevcut
hafif bağımlılıklarla güvenilir biçimde alınamıyorsa raporda `unavailable`
olarak kalır. Profilleme karşılaştırmalarında aynı video bölümü, kamera seti,
stride, model ve yapılandırma kullanılmalıdır.
...
- AIST Dance/AIST++ `CURRENT_VALIDATION` sınıfında ikincil kamera, triangulation, ViTPose inference ve SMPL smoke verisidir; aktif ürün workflow'u değildir.
...
WholeBody-133 ölçüm, hareket/faz kanıtı, otomatik duraklama, yanlış
hareket/duruş teşhisi, hareket bazlı teknik uygunluk ve kanıt güveni füzyonu,
puansız Presentation proxy'leri, RulePack kontrolü, Accuracy kararları ve HTML
inceleme raporunu üretir. BODY-17 hareket enerjisinden puansız otomatik hareket
penceresi ve faz ankrajı önerileri çıkarır; bunları onaylı timeline ile yalnız
sonradan karşılaştırır ve timeline'ı değiştirmez. Kaynak FFV1/AVI kameraları zaman çizelgesini
değiştirmeden H.264/MP4 tarayıcı kopyalarına dönüştürür; HTML bu nedenle ek
codec kurulumu istemeden gerçekten oynatılabilir. Ayrıca aynı profil ve kayıt
kapsamındaki eski koşuları karşılaştıran regresyon raporu üretir. Kinematik yanlış
hareket/duruş adayları `inferred` kalır ve otomatik `-0,3` kesinti oluşturmaz.
...
│   ├── automatic_segmentation_report.json
...
│   ├── categorical_diagnostics_report.json
│   ├── technical_conformance_report.json
│   ├── presentation_diagnostics_report.json
│   ├── run_history_report.json
│   └── movement_evidence_report.json
├── csv\
│   ├── wholebody_metrics.csv
│   ├── automatic_segmentation_signal.csv
...
├── videos\
│   ├── browser\
│   │   ├── zed_35151067_browser.mp4
│   │   └── zed_37137479_browser.mp4
│   ├── browser_review_video_manifest.json
...
2. `json/automatic_segmentation_report.json`: BODY-17 hareket sinyalinden
   bulunan hareket/faz sınırları ve onaylı timeline'a karşı kare/saniye
   hataları. Tanısaldır; timeline, puan veya kesintiyi değiştirmez.
3. `videos/poomsae_scoring_annotated.mp4`: iki kamerayı yan yana gösteren;
...
4. `json/source_bound_accuracy_decisions.json`: kaynak ve belirsizlik bağlı
   tek tek kararlar.
5. `json/categorical_diagnostics_report.json`: M01-M06 için beklenen ve
   alternatif teknik/duruş profillerini karşılaştıran, puansız kinematik
   kontroller.
6. `json/technical_conformance_report.json`: M01-M06 için doğru hareket/duruş
   kimliği, beklenen teknik ölçütler, `%95` belirsizlik ve kanıt kalitesini
   hareket bazında birleştirir. Sonuç yalnız `ölçülen kapsam` içindir; puan ve
   otomatik kesinti üretmez.
7. `json/presentation_diagnostics_report.json`: hız-güç, ritim-tempo ve enerji
   ifadesi proxy'leri; `total_score` her zaman `null`.
8. `review/poomsae_scoring_review.html`: zaman çizelgesi değişmeyen ham
   kameraların H.264 inceleme kopyalarını senkron izleme, video yükleme durumunu
   görme, `Videoda aç` ile kanıt anına gitme ve her karar için
...
9. `review/run_history.html`: aynı profil, M01-M06 kapsamı ve çalışma modundaki
   koşuları karşılaştırır. Yalnız aynı pose SHA-256 için ölçüm kapsamı ve
   readiness regresyon uyarıları üretir; aday sayısını kalite puanı saymaz.
10. `json/decision_evidence_events.json`: video ve HTML'nin kullandığı değişmez
   olay sözleşmesi; kare/zaman, ölçüm, `%95` aralığı, kaynak sınırı ve çizilecek
   eklem geometrisini taşır.
11. `csv/wholebody_metrics.csv`: bütün sayısal WholeBody-133 ölçümleri.
...
içindedir. Kinematik yanlış hareket/duruş teşhis sözleşmesi
[`docs/CATEGORICAL_DIAGNOSTICS.md`](docs/CATEGORICAL_DIAGNOSTICS.md) belgesindedir.
...
Kinematik taramanın yanlış hareket/duruş adayı üretmesi doğrudan gözlem
sayılmaz ve kesinti yaratmaz.
...
| AIST videoları ve AIST++ kamera verisi | `CURRENT_VALIDATION` çok-kamera smoke/regresyon testi | Opsiyonel |
...
AIST Dance Video DB videoları ikincil çok-kamera validation/smoke için kullanılabilir; güncel ZED/poomsae geliştirme hattının yerine geçmez. AIST++ annotation dosyaları COCO 17 eklem formatındadır; bu nihai COCO-WholeBody 133 hedefini değiştirmez. Videolar bizim ViTPose-Huge WholeBody adapter yoluna girdiğinde hedef yine 133 eklemdir. AIST++ 17 eklem verisi sadece calibration, projection, triangulation ve hata ölçümü için opsiyonel doğrulama verisidir.
...
-> TriangulatedPose3D
-> keypoints_3d_world[t, 133, 3]
-> PoomsaeSpec + MovementTimeline
-> WholeBody / kategorik / teknik teşhisler
-> kaynak-bağlı Accuracy kararları ve hazırlık kapıları
```

Puanlama verisi sürümlü YAML sözleşmeleri ve bunlara bağlı JSON raporlarıyla taşınır; ayrı bir sınıf hiyerarşisi yoktur. Kısmi kayıt ve teşhis çıktıları sayısal skor üretmez. Kaynak-bağlı karar akışı da hazırlık kapıları geçmeden resmî puan olarak kullanılamaz.
...
- `json/run_manifest.json`: Git durumu, komut satırı, Python/OS/paket/GPU ortamı, model ve config
  SHA-256 kayıtları, input dosya kimlikleri ve koşuya sabitlenmiş calibration snapshot bağlantısı
...
Yeni `vitpose_session_3d.json`, `run_quality_report.json` ve `run_manifest.json` dosyaları `schema_version: 1`
taşır. Sürümsüz tarihsel 3B/kalite artifact'ları yalnız `LEGACY_SUPPORTED` olarak okunur; mevcut provenance
alanları sonradan uydurulmaz. Bilinmeyen ileri şema sürümleri güvenli biçimde reddedilir.
