...
def triangulate_frame(
    ...
    if not np.isfinite(min_keypoint_score) or not 0.0 <= min_keypoint_score <= 1.0:
        raise ValueError("min_keypoint_score must be finite and between 0 and 1")
    if not np.isfinite(max_reprojection_error_px) or max_reprojection_error_px <= 0:
        raise ValueError("max_reprojection_error_px must be finite and positive")
    if max_hypotheses < 1:
    ...
    for joint_idx in range(COCO_WHOLEBODY_KEYPOINTS):
        ...
        for camera_id, pose in poses_by_camera.items():
            ...
            score = float(pose.scores[joint_idx])
            if not np.all(np.isfinite(point)) or not np.isfinite(score) or score < min_keypoint_score:
                continue
...
def triangulate_n_view(
    ...
    for index, (point, projection) in enumerate(zip(points_2d, projection_mats, strict=True)):
        ...
        projection_values = np.asarray(projection, dtype=float)
        if point_values.shape != (2,) or projection_values.shape != (3, 4):
            raise ValueError("Each point must have shape [2] and each projection matrix shape [3, 4]")
        x, y = point_values
        if not np.isfinite(x) or not np.isfinite(y):
            continue
        if not np.all(np.isfinite(projection_values)):
            continue
        if weights is not None:
            raw_weight = float(weights[index])
            if not np.isfinite(raw_weight) or raw_weight < 0.0:
                continue
            weight = float(np.sqrt(max(raw_weight, 1e-6)))
        else:
            weight = 1.0
        rows.append((x * projection_values[2, :] - projection_values[0, :]) * weight)
        rows.append((y * projection_values[2, :] - projection_values[1, :]) * weight)
    if len(rows) < 4:
        return None
...
def robust_triangulate_n_view(
    ...
    if min_views < 2:
        raise ValueError("min_views must be at least 2")
    if not np.isfinite(max_reprojection_error_px) or max_reprojection_error_px <= 0.0:
        raise ValueError("max_reprojection_error_px must be finite and positive")
    if max_hypotheses < 1:
        raise ValueError("max_hypotheses must be positive")
    ...
    ranking_scores = np.nan_to_num(
        np.clip(np.asarray(scores, dtype=float), 0.0, 1.0),
        nan=0.0,
        posinf=1.0,
        neginf=0.0,
    )
    ...
    pair_candidates.sort(
        key=lambda pair: ranking_scores[pair[0]] * ranking_scores[pair[1]],
        reverse=True,
    )
    ...
    for pair in pair_candidates:
        result = triangulate_n_view(
            ...
            [ranking_scores[index] for index in pair],
        )
        if result is None:
            continue
        point, _ = result
        ...
        confidence = float(np.mean(ranking_scores[inliers]))
        hypotheses.append((len(inliers), -median_error, confidence, inliers, point))
    if not hypotheses:
        return None
    _, _, _, inliers, _ = max(hypotheses, key=lambda item: item[:3])
    ...
    linear_result = triangulate_n_view(
        ...
        [ranking_scores[index] for index in inliers],
    )
    ...
    if refined_inliers != inliers:
        refined_linear = triangulate_n_view(
            [normalized_points_2d[index] for index in refined_inliers],
            [normalized_projection_mats[index] for index in refined_inliers],
            [ranking_scores[index] for index in refined_inliers],
        )
        if refined_linear is None:
            return None
        conditioning = refined_linear[1]
        point = refine_point_nonlinear(
            refined_linear[0],
